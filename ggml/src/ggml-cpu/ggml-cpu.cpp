...
static bool ggml_backend_cpu_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        case GGML_OP_W1A1_MUL_MAT: {
            ...
            if (!src0 || !scales || !acts || op->type != GGML_TYPE_F32 ||
                src0->type != GGML_TYPE_I32 || scales->type != GGML_TYPE_F32 || acts->type != GGML_TYPE_F32 ||
                !ggml_is_contiguous(src0) || !ggml_is_contiguous(scales) || !ggml_is_contiguous(acts)) {
                return false;
            }
            int64_t k;
        ...
        case GGML_OP_W4A4_MUL_MAT: {
            ...
            return src0 && scales && acts && op->type == GGML_TYPE_F32 &&
                src0->type == GGML_TYPE_I8 && scales->type == GGML_TYPE_F32 && acts->type == GGML_TYPE_F32 &&
                ggml_is_contiguous(src0) && ggml_is_contiguous(scales) && acts->nb[0] == sizeof(float) &&
                acts->ne[0] > 0 && src0->ne[0] == (acts->ne[0] + 1)/2 && src0->ne[1] > 0 &&
                src0->ne[2] == 1 && src0->ne[3] == 1 &&
                scales->ne[0] == src0->ne[1] && scales->ne[1] == 1 && scales->ne[2] == 1 && scales->ne[3] == 1 &&
                acts->ne[1] > 0 && acts->ne[2] == 1 && acts->ne[3] == 1 &&
                op->ne[0] == src0->ne[1] && op->ne[1] == acts->ne[1];
        }
        case GGML_OP_CPY:
